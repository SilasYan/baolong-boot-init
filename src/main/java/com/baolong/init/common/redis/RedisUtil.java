package com.baolong.init.common.redis;
...
public class RedisUtil {
	 ...
	 * @param <T>   泛型
	 */
	public <T> void set(final String key, final T value, final Integer time) {
	 ...
	 * @param <T>   泛型
	 */
	public <T> void set(final String key, final T value, final Integer time, final TimeUnit unit) {
	...
	public long increment(final String key, final long delta) {
		if (delta <= 0) throw new RuntimeException("递增因子必须大于 0");
	...
	public long decrement(final String key, final long delta) {
		if (delta <= 0) throw new RuntimeException("递减因子必须大于 0");
		Long increment = redisTemplate.opsForValue().increment(key, -delta);
		return increment != null ? increment : -1;
	}
	 ...
	 * @param <T>  泛型
	 */
	public <T> void hmSet(final String key, Map<String, T> map, final Integer time) {
		redisTemplate.opsForHash().putAll(key, map);
	 ...
	 * @param <T>  泛型
	 */
	public <T> void hmSet(final String key, Map<String, T> map, final Integer time, final TimeUnit unit) {
		redisTemplate.opsForHash().putAll(key, map);
	...
	public double hIncrement(final String key, final String item, final double delta) {
		if (delta <= 0) throw new RuntimeException("递增因子必须大于 0.0");
		return redisTemplate.opsForHash().increment(key, item, delta);
	}
	...
	public double hDecrement(final String key, final String item, final double delta) {
		if (delta <= 0) throw new RuntimeException("递减因子必须大于 0.0");
		return redisTemplate.opsForHash().increment(key, item, -delta);
	}
	 ...
	 * @return 集合数量
	 */
	public <T> long sSet(final String key, final Set<T> set, final Integer time) {
		Long count = redisTemplate.opsForSet().add(key, set);
		redisTemplate.expire(key, time, TimeUnit.SECONDS);
		return count == null ? 0 : count;
	}
	 ...
	 * @return 集合数量
	 */
	public <T> long sSet(final String key, final Set<T> set, final Integer time, final TimeUnit unit) {
		Long count = redisTemplate.opsForSet().add(key, set);
		redisTemplate.expire(key, time, unit);
		return count == null ? 0 : count;
	}
	 ...
	 * @return 集合数量
	 */
	public <T> long sSet(final String key, final T... values) {
	 ...
	 * @return 集合数量
	 */
	public <T> long sSet(final String key, final Integer time, final T... values) {
		Long count = redisTemplate.opsForSet().add(key, values);
		redisTemplate.expire(key, time, TimeUnit.SECONDS);
		return count == null ? 0 : count;
	}
	 ...
	 * @return 集合数量
	 */
	public <T> long sSet(final String key, final Integer time, final TimeUnit unit, final T... values) {
		Long count = redisTemplate.opsForSet().add(key, values);
		redisTemplate.expire(key, time, unit);
		return count == null ? 0 : count;
	}
	 ...
	 * @return 集合数量
	 */
	public <T> long lSet(final String key, final List<T> list, final Integer time) {
		Long count = redisTemplate.opsForList().rightPushAll(key, list);
		redisTemplate.expire(key, time, TimeUnit.SECONDS);
		return count == null ? 0 : count;
	}
	 ...
	 * @return 集合数量
	 */
	public <T> long lSet(final String key, final List<T> list, final Integer time, final TimeUnit unit) {
		Long count = redisTemplate.opsForList().rightPushAll(key, list);
		redisTemplate.expire(key, time, unit);
		return count == null ? 0 : count;
	}
